...
const UpgradeBanner = ({ title, desc }) => (
  <div className="max-w-md w-full text-center card-ats-bordered border-amber-200/80 bg-amber-50/30 p-10 mx-auto">
    <div className="w-14 h-14 rounded-2xl bg-amber-100 flex items-center justify-center mx-auto mb-4">
      <Lock className="w-7 h-7 text-amber-600" />
    </div>
    <h2 className="text-xl font-bold text-stone-900">{title}</h2>
    <p className="text-stone-500 mt-2 text-sm">{desc}</p>
  ...
  return (
    <div className="fixed inset-0 bg-stone-900/55 backdrop-blur-sm z-50 flex items-center justify-center p-4">
      <div className="relative bg-white rounded-2xl sm:rounded-3xl w-full max-w-lg border border-stone-200/60 shadow-2xl modal-panel-ats overflow-hidden p-6">
        ...
        <div className="mt-4 flex items-center gap-2 bg-stone-50 border border-stone-200 rounded-lg p-3">
    ...
    <div className="fixed inset-0 bg-stone-900/55 backdrop-blur-sm z-50 flex items-center justify-center p-4">
      <div className="relative bg-white rounded-2xl sm:rounded-3xl w-full max-w-lg max-h-[85vh] flex flex-col border border-stone-200/60 shadow-2xl modal-panel-ats overflow-hidden">
        <div className="p-5 border-b border-stone-100">
          <h3 className="text-lg font-bold text-stone-900">New Webhook Endpoint</h3>
        </div>
        <div className="p-5 space-y-4 overflow-y-auto">
          <div>
            <label className="text-sm font-medium text-stone-700">Endpoint URL</label>
            <input value={url} onChange={(e) => setUrl(e.target.value)} placeholder="https://example.com/webhooks/ats" className="mt-1 input-ats" />
          </div>
          <div>
            <label className="text-sm font-medium text-stone-700">Description</label>
            <input value={description} onChange={(e) => setDescription(e.target.value)} placeholder="Optional" className="mt-1 input-ats" />
          </div>
          <div>
            <label className="text-sm font-medium text-stone-700 mb-2 block">Events ({events.length} selected)</label>
            <div className="grid grid-cols-2 gap-1.5 max-h-56 overflow-y-auto border border-stone-100 rounded-lg p-3">
              {availableEvents.map((evt) => (
                <label key={evt} className="flex items-center gap-2 text-xs text-stone-700 cursor-pointer">
                  <input type="checkbox" checked={events.includes(evt)} onChange={() => toggle(evt)} className="rounded border-stone-300" />
                  {evt}
                </label>
              ))}
            </div>
...
const ApiKeyModal = ({ canWrite, onClose, onSave, saving }) => {
  ...
  return (
    <div className="fixed inset-0 bg-stone-900/55 backdrop-blur-sm z-50 flex items-center justify-center p-4">
      <div className="relative bg-white rounded-2xl sm:rounded-3xl w-full max-w-md border border-stone-200/60 shadow-2xl modal-panel-ats overflow-hidden p-6">
        <h3 className="text-lg font-bold text-stone-900">New API Key</h3>
        <div className="mt-4 space-y-4">
          <div>
            <label className="text-sm font-medium text-stone-700">Key name</label>
            <input value={name} onChange={(e) => setName(e.target.value)} placeholder="e.g. Zapier integration" className="mt-1 input-ats" />
          </div>
          <label className={`flex items-center gap-2 text-sm ${canWrite ? 'text-stone-700 cursor-pointer' : 'text-stone-400'}`}>
            <input type="checkbox" checked={write} disabled={!canWrite} onChange={(e) => setWrite(e.target.checked)} className="rounded border-stone-300" />
...
const DeliveryLog = ({ endpointId }) => {
  ...
  if (deliveries === null) return <div className="p-4 text-sm text-stone-400 flex items-center gap-2"><Loader2 className="w-3.5 h-3.5 animate-spin" /> Loading deliveries…</div>;
    ...
    <div className="page-shell-ats max-w-4xl">
        ...
        <section>
          <div className="flex items-center justify-between mb-3">
            <h2 className="text-sm font-semibold text-stone-900 uppercase tracking-wider flex items-center gap-2"><Webhook className="w-4 h-4" /> Webhook Endpoints</h2>
            <button type="button" onClick={() => setShowWebhookModal(true)} className="btn-primary !px-3 !py-1.5 !text-sm">
              ...
              <div key={ep._id}>
                <div className="p-4 flex items-center justify-between">
                  <div className="flex-1 min-w-0">
                    <div className="font-medium text-stone-900 truncate">{ep.url}</div>
                  ...
                  <div className="flex items-center gap-1 shrink-0">
                    <span className={`text-xs px-2 py-0.5 rounded-full mr-2 ${ep.isActive ? 'bg-green-50 text-green-700' : 'bg-stone-100 text-stone-500'}`}>{ep.isActive ? 'Active' : 'Disabled'}</span>
                    <button onClick={() => setExpandedEndpoint(expandedEndpoint === ep._id ? null : ep._id)} className="p-2 hover:bg-stone-100 rounded-lg text-stone-500" title="View deliveries">
                      {expandedEndpoint === ep._id ? <ChevronUp className="w-4 h-4" /> : <ChevronDown className="w-4 h-4" />}
                    </button>
                    <button onClick={() => toggleWebhook(ep)} className="p-2 hover:bg-stone-100 rounded-lg text-stone-500" title={ep.isActive ? 'Disable' : 'Enable'}><Power className="w-4 h-4" /></button>
                    <button onClick={() => rotateSecret(ep)} className="p-2 hover:bg-stone-100 rounded-lg text-stone-500" title="Rotate secret"><RefreshCw className="w-4 h-4" /></button>
                ...
                {expandedEndpoint === ep._id && <div className="border-t border-stone-100 bg-stone-50/50"><DeliveryLog endpointId={ep._id} /></div>}
              </div>
            ))}
          </div>
        </section>
        ...
        <section>
          <div className="flex items-center justify-between mb-3">
            <h2 className="text-sm font-semibold text-stone-900 uppercase tracking-wider flex items-center gap-2"><KeyRound className="w-4 h-4" /> API Keys</h2>
            <button type="button" onClick={() => setShowKeyModal(true)} className="btn-primary !px-3 !py-1.5 !text-sm">
          ...
          <div className="card-ats-bordered divide-y divide-stone-100 overflow-hidden">
            {apiKeys.length === 0 ? (
              ...
              <div key={key._id} className="p-4 flex items-center justify-between">
                <div>
                  <div className="font-medium text-stone-900">{key.name}</div>
                  <div className="text-xs text-stone-400 mt-0.5 font-mono">{key.keyPrefix}••••••••</div>
                  <div className="text-xs text-stone-400 mt-0.5">Scopes: {key.scopes.join(', ')} {key.lastUsedAt && `· Last used ${new Date(key.lastUsedAt).toLocaleDateString()}`}</div>
                </div>
                <button onClick={() => revokeApiKey(key)} className="p-2 hover:bg-red-50 rounded-lg text-red-500"><Trash2 className="w-4 h-4" /></button>
              </div>
            ))}
          </div>
          <p className="text-xs text-stone-400 mt-2">Use as <code>Authorization: Bearer &lt;key&gt;</code> against <code>/api/v1/public/*</code>.</p>
        </section>
