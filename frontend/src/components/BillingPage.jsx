...
const MetricRing = ({ label, current, max, color, icon: Icon }) => {
  ...
  return (
    <div className="card-ats-bordered p-5 flex items-center justify-between">
      <div className="space-y-1">
        ...
        <div className="text-2xl font-bold text-stone-900">
          {(current || 0).toLocaleString()} <span className="text-sm font-normal text-stone-400">/ {unlimited ? '∞' : max.toLocaleString()}</span>
        </div>
      </div>
...
export default function BillingPage() {
  ...
  return (
    <div className="page-shell-ats max-w-6xl">
        ...
        >
          <div className="flex items-center gap-2 text-sm text-emerald-700 bg-emerald-50 px-3 py-1.5 rounded-xl border border-emerald-200 font-medium">
        ...
        {/* Current Plan Alert */}
        <div className="bg-gradient-to-r from-stone-900 to-slate-800 rounded-2xl p-6 md:p-8 text-white shadow-xl relative overflow-hidden">
          <div className="absolute top-0 right-0 w-64 h-64 bg-white/5 rounded-full -translate-y-1/2 translate-x-1/3 blur-3xl"></div>

          <div className="relative z-10 flex flex-col md:flex-row md:items-center justify-between gap-6">
            <div>
              <div className="flex items-center gap-3 mb-2">
                <span className="bg-brand-500/20 text-brand-300 border border-brand-500/30 px-2.5 py-1 rounded text-xs font-semibold uppercase tracking-wider">
                  Current Plan
                </span>
            ...
            <div className="flex items-center gap-4">
              {currentPlanMeta?.price != null && (
                <div className="text-right hidden md:block">
                  <div className="text-2xl font-bold">${currentPlanMeta.price} <span className="text-sm font-normal text-stone-400">/mo</span></div>
                </div>
              )}
              <button
        ...
        {/* Usage Stats */}
        <div>
          <h3 className="text-sm font-semibold text-stone-900 uppercase tracking-wider mb-4">Current Usage</h3>
          <div className="grid sm:grid-cols-2 lg:grid-cols-4 gap-4">
            <MetricRing label="Active Jobs" current={usage.jobs || 0} max={limits.maxJobs} color="text-brand-500" icon={Briefcase} />
            <MetricRing label="Team Members" current={usage.users || 0} max={limits.maxUsers} color="text-brand-500" icon={Users} />
        ...
        {/* Plans */}
        <div className="pt-8">
          <h3 className="text-2xl font-bold text-center text-stone-900 mb-8">Ready to grow? Upgrade your plan.</h3>
          <div className="grid md:grid-cols-3 gap-6 max-w-5xl mx-auto">
            {plans.filter((p) => p.id !== 'free_trial').map((plan) => {
              ...
              return (
                <div
                  key={plan.id}
                  className={`card-ats-bordered p-6 relative ${isCurrent ? '!border-2 !border-brand-500' : ''}`}
                >
                  {isCurrent && (
                  ...
                  <div className="mt-2 text-3xl font-bold text-stone-900">
                    {plan.price != null ? <>${plan.price}<span className="text-lg font-normal text-stone-500">/mo</span></> : 'Custom'}
                  </div>
