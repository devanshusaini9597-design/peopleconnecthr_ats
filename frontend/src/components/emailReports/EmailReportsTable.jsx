...
function cellValue(row, colId) {
  ...
  switch (colId) {
    ...
    case 'opened':
      return (
        <span className="tabular-nums text-teal-800">
          {t.opened ?? 0}
          <span className="text-xs text-stone-400"> · {r.openRate ?? 0}%</span>
        </span>
      );
    case 'clicked':
      return (
        <span className="tabular-nums text-indigo-800">
          {t.clicked ?? 0}
          <span className="text-xs text-stone-400"> · {r.clickRate ?? 0}%</span>
        </span>
      );
    case 'bounced':
    ...
    case 'openRate':
      return <span className="tabular-nums text-stone-800">{r.openRate ?? 0}%</span>;
    case 'clickRate':
      return <span className="tabular-nums text-stone-800">{r.clickRate ?? 0}%</span>;
    default:
      return '—';
  }
}
      ...
      <div className="flex flex-wrap items-center justify-between gap-3 border-b border-stone-100 px-4 py-3">
        <div>
          ...
          <p className="text-xs text-stone-500">
            Drag horizontally to scroll · metrics match the funnel above
          </p>
        </div>

        <div className="relative" data-tour="email-reports-columns">
          <button
            type="button"
