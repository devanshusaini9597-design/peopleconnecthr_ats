...
/** Public (token) proctoring events during assessment take */
export async function POST(req: NextRequest, ctx: Ctx) {
  try {
    ...
    if (type === 'window_blur') blurEvents.push({ at });
    ...
    const updated = await prisma.proctoringSession.update({
      ...
      data: {
        ...
        timeline: timeline.slice(-500) as unknown as object[],
        riskScore,
        flagged,
      },
    });
